...
class DBM:
    ...
    def learn(self, Xd):
        ...
        # Parameter update
        self.W[0] += self.lr*(np.tensordot(X[0]-self.O[0], X[1]-self.O[1], axes=([0],[0])) -
                        np.tensordot(self.X[0]-self.O[0], self.X[1]-self.O[1], axes=([0],[0])))/len(Xd) -\
						l2penalty*self.W[0]
        for i in range(1, len(self.W)):
            self.W[i] += self.lr*(numpy.dot((X[i]-self.O[i]).T,     X[i+1]-self.O[i+1]) -
                             numpy.dot((self.X[i]-self.O[i]).T, self.X[i+1]-self.O[i+1]))/len(Xd) -\
						l2penalty*self.W[i]
        for i in range(0, len(self.B)):
            self.B[i] += self.lr*(X[i]-self.X[i]).mean(axis=0)
    ...
    def pll_err(self, Xd, nSamples=100, burnin=100, skip=1):
        # Initialize a data particle
        numInput = len(Xd)
        X = [Xd]+[np.zeros([numInput,self.M[l]]) +self.O[l]
                           for l in range(1, len(self.M))]
